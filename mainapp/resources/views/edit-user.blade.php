    ...
    <div class="col-lg-12 pl-lg-5 pb-3 py-lg-5">
      <form action="/edit-user/{{$user->id}}" method="POST" id="registration-form">
        ...
        <div class="form-group">
          ...
          <input value="{{old('email', $user->email)}}" name="email" id="email-register" class="form-control" type="text" placeholder="you@example.com" autocomplete="off" />
          @error('email')
            <p class='m-0 small alert alert-danger shadow-sm'>{{$message}}</p>
          @enderror
        </div>
        ...
        <div class="form-group">
          ...
          <input name="password" id="password-register" class="form-control" type="password" placeholder="Enter new password" />
          @error('password')
            <p class='m-0 small alert alert-danger shadow-sm'>{{$message}}</p>
          @enderror
        </div>
